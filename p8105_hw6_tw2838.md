...
Now, I propose the model for birthweight as a linear regression model
which have `gaweeks`(gestational age in weeks), `malform` (presence of
malformations, 1 = present, 0 = absent), and `wtgain` (mother’s weight
gain during pregnancy) as predicting variables.
